...
def main(cfg):
    ...
    dataset, loader = create_dataset(
        Path(get_original_cwd()).joinpath('data'), 'train',
        batch_size=cfg.data.batch_size, seq_per_img=1, num_workers=cfg.data.num_workers)
    val_dataset, val_loader = create_dataset(
        Path(get_original_cwd()).joinpath('data'), 'val',
        batch_size=cfg.data.batch_size, seq_per_img=1, num_workers=cfg.data.num_workers)
    ...
    model = MCCFormer(
        ...
        pe_type=cfg.model.pe_type, max_len=dataset.max_seq_length)
    model.to(device)
    ...
    if cfg.test_only:
        evaluate(model, val_loader, device, cfg.optim.epochs, cfg.optim.print_freq, logger=logger)
        return
    ...
    for epoch in range(start_epoch, cfg.optim.epochs + 1):
        ...
        loss_avg = train_one_epoch(
            ...
            device, epoch, cfg.optim.print_freq, logger=logger)
