...
# https://github.com/cvpaperchallenge/Describing-and-Localizing-Multiple-Change-with-Transformers/blob/main/code/datasets.py
...
class CaptionDataset(Dataset):
    def __init__(self, data_folder, data_name, split, captions_per_image, dataset_name):
        ...
        with open(
            os.path.join(data_folder, self.split + "_CAPTIONS_" + data_name + ".json"), "r"
        ) as f:
            self.captions = json.load(f)

        with open(
            os.path.join(data_folder, self.split + "_CAPLENS_" + data_name + ".json"), "r"
        ) as f:
