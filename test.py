...
def main(cfg):
    ...
    test_dataset, test_loader = create_dataset(
        Path(get_original_cwd()).joinpath('data'), 'test',
    ...
    model = MCCFormer(
        ...
        pe_type=cfg.model.pe_type, max_len=test_dataset.max_seq_length)
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_loader), 1):
            d_features, n_features, q_features, targets, neg_targets, _, _, _, _, \
                d_img_paths, ncs_img_paths, sc_feats = data
